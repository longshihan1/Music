package com.longshihan.lightly.music.utils.util.storage;
...
import com.longshihan.lightly.music.utils.util.LogUtils;
import com.longshihan.lightly.music.utils.util.json.JSONHelper;
			...
			if (isSingle(type)) {
				...
				if (isObject(type)) {
					...
					try {
						Object temp = fields[i].get(o);
						if (null != temp)
							editor.putString(name, JSONHelper.toJSON(temp));
					} catch (IllegalAccessException e) {
						LogUtils.e(TAG, e);
					}
				}
			...
			if (isSingle(type)) {
				...
				if (isObject(type)) {
					...
					if (!TextUtils.isEmpty(tempValue)) {
						Object tempObj = null;
						if (isArray(type))
							tempObj = JSONHelper.parseArray(tempValue, type);
						else if (isCollection(type)) {
							tempObj = JSONHelper.parseCollection(tempValue, type, JSONHelper.getGeneric(fields[i]));
						} else if (isMap(type)) {
							// tempObj=JSONHelper.parseCollection()
						}
						if (null != tempObj) {
							fields[i].setAccessible(true);
							try {
